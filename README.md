...
```
速度控制优化参数，用于表示控制要求预期行驶到的目标点距离约束条件下行驶路径建距离的权重，权重越高，表示行走越
准确，但行驶平滑程度越低。（数值要求大于等于0）
```

~endpoint_weight (double, default: 1.0)

```
速度控制优化参数，用于表示车辆在约束条件下预期行驶到的目标点和控制要求预期行驶到的目标点间距离的权重，权重越
高，表示行驶线速度越接近指定线速度（数值要求大于0）
```

~angle_weight (double, default: 0.5)

```
速度控制优化参数，用于表示车辆在约束条件下预期行驶到的目标点和控制要求预期行驶到的目标点间航向角差距的权重，
权重越高，表示行驶角速度越接近指定角速度（数值要求大于0）
```
